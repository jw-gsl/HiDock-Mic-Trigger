...
| Plaud account: Sign out (keep) vs Forget (remove) in Device Manager | `DeviceRowView` Sign out / Sign in + Forget; `signOutPlaud` clears Keychain session, keeps device | `DeviceRowWidget` Sign out / Sign in + Forget; clears account tokens, keeps `PairedDevice` | Both | 2026-06-23 — cloud account gets a reversible Sign out (session cleared, account stays linked) distinct from Forget (removed entirely); reuses the existing "signed-out" state |
