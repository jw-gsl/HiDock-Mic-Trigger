#!/usr/bin/env python3
"""HiDock USB extractor — Windows-compatible version.
...
Windows notes:
- Requires WinUSB or libusb-win32 driver bound to the HiDock device
  (use Zadig to install: https://zadig.akeo.ie/)
- Uses libusb-package to bundle libusb DLLs automatically
- Kernel driver detach is not applicable on Windows
- Temp files use the system temp directory via tempfile.gettempdir()
"""
...
# On Windows, ensure libusb DLLs are available via libusb-package before
# importing pyusb.  This is a no-op on non-Windows platforms.
...
def prepare_device(dev):
    ...
    # Kernel driver detach is only relevant on Linux/macOS.
    # On Windows with WinUSB/libusb-win32 this is not needed.
    if platform.system() != "Windows":
        try:
